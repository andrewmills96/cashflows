...
class Bonds:
    ...
    def run_sim(
        ...
        * PV calculation uses a precomputed lookup table keyed on the unique
        ...
        # Broadcast one_plus_y to (n_years_cf, n_years) once, outside the loop
        one_plus_y_mat = one_plus_y[:, np.newaxis] * np.ones((1, n_years))   # (n_years_cf, n_years)
        ...
        for si, s in enumerate(unique_spreads):
            ...
            # Compute only on future cells to avoid NaN propagation from np.where
            # eager evaluation. Both arrays are (n_years_cf, n_years) so boolean
            # indexing with future (same shape) is valid.
            dfs_mat = np.zeros_like(dtime_mat)
            dfs_mat[future] = (one_plus_y_mat[future] + s) ** (-dtime_mat[future])
